...
export async function GET(req: NextRequest) {
  try {
    ...
    if (run.status === "DRAFT") {
      return NextResponse.json({ error: "Payroll must be PROCESSED or LOCKED to download payslips" }, { status: 400 });
    }
    ...
    const pdfBuffer = await generateBulkPayslipsPdf(typedDetails, entityName, monthName, year);

    const safeEntity = entityName.replace(/[^a-zA-Z0-9]/g, "_");
    const filename = `${safeEntity}_Payslips_${monthName}_${year}.pdf`;
    ...
    console.error("payslips-bulk GET error:", error);
